 ...
 *
 */
function fs_use_archive_loop() {

	if( ! is_singular() && ! is_404() ) {
		add_action( 'genesis_loop', 'fs_archive_loop' );
		remove_action( 'genesis_loop', 'genesis_do_loop' );
	}
}
add_action( 'template_redirect', 'fs_use_archive_loop', 20 );

/**
 * Archive Loop
 * Uses template partials
 */
function fs_archive_loop() {
	...
	if ( have_posts() ) {
		...
		while ( have_posts() ) {
			...
			// Template part
			$partial = apply_filters( 'fs_loop_partial', 'archive' );
			$context = apply_filters( 'fs_loop_partial_context', is_search() ? 'search' : get_post_type() );
...
function be_remove_entry_title() {
	...
	if( $has_h1 ) {
		...
		remove_action( 'genesis_entry_header', 'fs_entry_category', 8 );
		remove_action( 'genesis_entry_header', 'genesis_do_post_title' );
		remove_action( 'genesis_entry_header', 'fs_entry_author', 12 );
		remove_action( 'genesis_entry_header', 'fs_entry_header_share', 13 );
